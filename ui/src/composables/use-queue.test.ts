...
import {
  ...
  stopQueueDispatcher,
  useQueue
} from '@composables/use-queue'
import { clearToasts, useToasts } from '@composables/use-toasts'
import { resetTools } from '@composables/use-tools'
import { __resetTurnEndedListeners, pushTurnEnded, pushTurnStarted, resetTurns } from '@composables/use-turns'
...
describe('startQueueDispatcher', () => {
  ...
  it('flushes the queue on stopReason=cancelled and emits a toast', () => {
    ...
    const messages = useToasts().entries.value.map((t) => t.message)
