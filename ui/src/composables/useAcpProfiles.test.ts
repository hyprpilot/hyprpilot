...
import { useAcpProfiles } from '@composables/useAcpProfiles'
...
function host() {
  return defineComponent({
    setup(_, { expose }) {
      const composable = useAcpProfiles()
      expose(composable)
...
describe('useAcpProfiles', () => {
  ...
  it('refresh() re-fetches and updates the reactive list', async () => {
    ...
    await (wrapper.vm as unknown as ReturnType<typeof useAcpProfiles>).refresh()
    await wrapper.vm.$nextTick()
  ...
  it('select() persists the id to localStorage and next mount restores it', async () => {
    ...
    await flushAsync()
    await wrapper.vm.$nextTick()
    ;(wrapper.vm as unknown as ReturnType<typeof useAcpProfiles>).select('strict')
  ...
  it('ignores select() for ids not in the current list', async () => {
    ...
    await flushAsync()
    await wrapper.vm.$nextTick()
    ;(wrapper.vm as unknown as ReturnType<typeof useAcpProfiles>).select('ghost')
