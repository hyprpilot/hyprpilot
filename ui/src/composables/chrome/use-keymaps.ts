...
export function useKeymap(target: MaybeRefOrGetter<EventTarget | null | undefined>, entries: () => KeymapEntry[]): void {
  ...
  const listener = (e: Event): void => {
    ...
    for (const entry of entries()) {
      ...
      // a user TOML omits a key under a nested namespace. Without this
      // guard, the FIRST missing binding on the list crashes
      // `matchesBinding` on `undefined.modifiers`, the for-loop
      // aborts, and every subsequent entry (e.g. `palette.open`) goes
      // unhandled — dropping the user out of the keymap entirely.
      if (!entry.binding) {
        continue
      }
