...
import { __resetPaletteStackForTests, PaletteMode, usePalette, useActiveInstance, __resetAllSessionInfoForTests, clearToasts, useToasts } from '@composables'
...
describe('openModesLeaf', () => {
  it('asks the daemon to ensure-spawn when no active instance is set', async() => {
    ...
    expect(top?.entries.map((e) => e.id)).toEqual(['plan'])
  })
