...
vi.mock('@composables/use-active-instance', () => ({
  useActiveInstance: () => ({
    id: activeInstanceRef
  })
}))

vi.mock('@composables/use-home-dir', () => ({
  useHomeDir: () => ({
    homeDir: ref('/home/cenk')
  })
}))

vi.mock('@composables/use-phase', () => ({
  usePhase: () => ({
    phase: computed(() => 'idle')
  })
}))

vi.mock('@composables/use-queue', () => ({
  useQueue: () => ({
    items: computed(() => [])
  })
}))

vi.mock('@composables/use-session-info', () => ({
...
vi.mock('@composables/use-terminals', () => ({
  useTerminals: () => ({
    all: computed(() => [])
  })
}))

vi.mock('@composables/use-toasts', () => ({
...
describe('openInstancesLeaf', () => {
  ...
  it('lists every live instance with agent + profile in the row name', async () => {
    invokeMock.mockResolvedValue({
      instances: [
        { agentId: 'claude-code', profileId: 'ask', instanceId: 'inst-A' },
        { agentId: 'codex', profileId: 'plan', instanceId: 'inst-B' }
      ]
    })
  ...
  it('marks the active instance in its description prefix', async () => {
    ...
    invokeMock.mockResolvedValue({
      instances: [{ agentId: 'claude-code', profileId: 'ask', instanceId: 'inst-A' }]
    })
