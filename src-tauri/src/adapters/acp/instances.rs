...
// The pure config→resolution core (profile resolution, patch
// folding, MCP/skills registry construction) lives in `crate::resolve`
// (K-725) so the launcher (`adapters::cli`) can consume it without
// depending on this module. Re-exported here so every daemon/ACP
// caller in this file (and its `mod tests`) keeps compiling
// unchanged — `crate::resolve` uses `anyhow::Error`, not `RpcError`.
// The `_for_spawn` variants have no production caller left in this
// file (the launcher now imports them straight from `crate::resolve`)
// but stay re-exported for `mod tests` below.
#[allow(unused_imports)]
pub(crate) use crate::resolve::{
    apply_mcp_glob_defaults, build_mcp_registry_with, build_skills_registry_with, effective_mcp_files_with,
    effective_mcp_with, prepend_auto_mcp_definition, resolve_effective_profile, resolve_effective_profile_for_spawn,
    resolve_into_instance_and_profile, resolve_into_instance_and_profile_for_spawn,
};
