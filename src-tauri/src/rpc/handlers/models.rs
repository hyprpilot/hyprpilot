...
/// `models/*` namespace: `models/list`, `models/set`. Mirrors
/// `modes/*` — instance-scoped, membership-checked, stubbed past
/// the check until `AcpInstance` caches `available_models` +
/// the runtime actor grows a `SetModel` command (ref K-251).
pub struct ModelsHandler;

#[async_trait]
impl RpcHandler for ModelsHandler {
        ...
        match method {
            "models/list" => {
                let InstanceIdOnly { instance_id } = serde_json::from_value(params)
                    .map_err(|err| RpcError::invalid_params(format!("models/list params: {err}")))?;
                let _ = adapter.contains_instance(&instance_id).await?;
                Err(RpcError::internal_error("models/list not implemented — ref K-251"))
            }
            "models/set" => {
                let SetModelParams {
                    instance_id,
                    ...
                    .map_err(|err| RpcError::invalid_params(format!("models/set params: {err}")))?;
                let _ = adapter.contains_instance(&instance_id).await?;
...
mod tests {
    ...
    async fn dispatch(method: &str, params: Value) -> Value {
        ...
        match ModelsHandler.handle(method, params, ctx).await {
    ...
    #[tokio::test]
    async fn models_list_missing_instance_id_is_invalid_params() {
        let v = dispatch("models/list", json!({})).await;
    ...
    #[tokio::test]
    async fn models_unknown_verb_is_method_not_found() {
        let v = dispatch("models/bogus", Value::Null).await;
