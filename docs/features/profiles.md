...
The field-by-field reference lives at [Configuration → `[profile]` & `[[profiles]]`](../configuration/profiles).
...
MCP servers extend an agent with tools; skills attach markdown context. Both have their own pages — see [MCP Catalogue](./mcp) and [Skills](./skills). In short:

- `mcps = [ … ]` on a profile is a per-profile MCP catalogue that wholesale-replaces the shared set; `mcps = []` means "no MCPs".
- The in-tree `hyprpilot` MCP server (which delivers your skills) is configured under a `[mcp]` block, seeded globally via `[[patches]]` and overridable per-profile.
