...
- **Per-page Markdown** — every page is also emitted as raw Markdown at its path with a `.md` suffix (e.g. `/config/profiles.md`), so an agent can fetch just the page it needs.
...
- To answer a question about a single topic, fetch that page's Markdown — e.g. `https://hyprpilot.kilic.dev/runtime/skills.md`.
